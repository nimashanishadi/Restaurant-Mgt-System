                    ...
                    <table id="example" class="table" cellspaceing="0" style="width:100%">
                      ...
                      @foreach($items as $item)
                        <tr>
                            ...
                            <td><img class ="img-responsive img-thumbnail" src="{{asset('storage/upload/'.$item->image)}}" style="height:200px; width: 800px" alt=""></td>
                            ...
                            <td><a href="{{ route('item.edit', $item->id) }}" class="btn btn-info btn-sm"><i class="material-icons">mode_edit</i></a>
                              <form  id="delete-form-{{$item->id}}" action="{{route('item.destroy',$item->id)}}" style="display:none;" method="POST">
                                ...
                                <button type="button" class="btn btn-danger btn-sm" onclick="if(confirm('Are you sure, You wanna delete this?')){event.preventDefault();document.getElementById('delete-form-{{$item->id}}').submit();}
                                else{
